...
// окно демо проектами
class MyUIBlocksWindDivProjectDemo
{
	container;
	content;
	wrapDemo;
	...
	html_1()
	{
		...
		`<div style="display: none;"> 
			<div style="${cssHeader}"><strong>Демо проекты</strong></div>
	...
	{		
		...
		btnPr1.onmousedown = () => { myUIBlocksWindMain.closeWin(); loadFile({local: 'demo/1.json'}); }
		btnPr2.onmousedown = () => { myUIBlocksWindMain.closeWin(); loadFile({local: 'demo/2.json'}); }
		btnPr3.onmousedown = () => { myUIBlocksWindMain.closeWin(); loadFile({local: 'demo/3.json'}); }
		btnPr4.onmousedown = () => { myUIBlocksWindMain.closeWin(); loadFile({local: 'demo/4.json'}); }
		btnPr5.onmousedown = () => { myUIBlocksWindMain.closeWin(); loadFile({local: 'demo/5.json'}); }
	}	
