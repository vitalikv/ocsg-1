		...
		parse: function ( FBXBuffer, path ) {
			...
			if ( isFbxFormatBinary( FBXBuffer ) ) {
				...
				if ( getFbxVersion( FBXText ) < 7000 ) {
					...
					Build.errInf({show: true, inf: 'FBX version not supported'});
	...
	FBXTreeParser.prototype = {
		...
		loadTexture: function ( textureNode, images ) {
			...
			if ( extension === 'tga' ) {
				...
				let item = INPL.arrInd.find((item) => item.name === fileName);

				if(item) 
				{
					texture = new Texture();
					texture.image = new Image();
					texture.image.src = item.result;	
					texture.needsUpdate = true;

					texture.image.onload = () => { Build.render(); }
				}
				else
				{
					if(!fileName)
		...
		parseParameters: function ( materialNode, textureMap, ID ) {
			...
			connections.get( ID ).children.forEach( function ( child ) {
				...
				switch ( type ) {
					...
					case '3dsMax|CoronaMtlPb|texmapDiffuse':
					case '3dsMax|params|texmap':
					...
					case '3dsMax|maps|texmap_opacity':
					...
					default:
						let map = scope.getTexture( textureMap, child.ID );
						if(map)
						{
							parameters.map = map;
							parameters.map.encoding = sRGBEncoding;												
