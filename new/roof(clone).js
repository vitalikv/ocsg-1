...
class Roof 
{
	...
	clickRoof(cdm)
	{
		...
		if(camera == cameraTop) { myComposerRenderer.outlineRemoveObj(); }
		if(camera == camera3D) { myComposerRenderer.outlineAddObj({arr: [obj]}); }
	...
	{		
		...
		for(let i = 0; i < group.length; i++)
		{
			...
			g.merge(group[i].geometry.clone(), group[i].matrixWorld);
			g.mergeVertices();
		...
		let o = new THREE.Mesh(g, new THREE.MeshStandardMaterial({ color: 0x00ff00, side: 2 }));
			//o.position.copy( roof.position );
			//o.quaternion.copy( roof.quaternion );
			//o.scale.copy( roof.scale );	
o.updateMatrix();			
		console.log(999, o);
		//scene.add(o);
		this.cutMeshBSP(o);
	...
	{  
		...
		for ( let i = 0; i < w.length; i++ )
		{
			...
			w[i].geometry.groupsNeedUpdate = true;
			w[i].geometry.normalsNeedUpdate = true;
			w[i].geometry.uvsNeedUpdate = true;
w[i].geometry.verticesNeedUpdate = true;		
